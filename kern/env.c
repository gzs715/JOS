...
envid2env(envid_t envid, struct Env **env_store, bool checkperm)
{
	...
	if (e->env_status == ENV_FREE || e->env_id != envid) {
		cprintf("e->env_id:%08x,envid:%08x\n",e->env_id,envid);
		*env_store = 0;
		return -E_BAD_ENV;
	}
...
segment_alloc(struct Env *e, void *va, size_t len)
{
	...
	//cprintf("begin segment alloc,len=%08x,va=%08x\n",len,va);
	void* rounddown_va = ROUNDDOWN(va, PGSIZE);
	size_t roundup_len = ROUNDUP(len, PGSIZE);
	int i,r;
	struct Page *seg_page; 
	for(i = 0;i< roundup_len;i += PGSIZE)
	{
		if((r=page_alloc(&seg_page)) == -E_NO_MEM)
			panic("Alloc segment failed because of no memory");
		else
		{
			if((r=page_insert(e->env_pgdir, seg_page, rounddown_va + i, PTE_U|PTE_W|PTE_P)) != 0)
				panic("segment alloc: %e",r);
		}
	}
	//cprintf("segment alloc success\n");
}
...
{
	//in the order of like the struct Trapframe
	//iret will pop the eip,cs and eflags
